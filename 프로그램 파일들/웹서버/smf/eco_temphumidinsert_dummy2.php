...
{ // check if both fields are set
	...
	$sql3 = "SELECT * FROM temphumid_v01 where device_id = '$device_id' and hour_time = '$hour_time' ORDER BY `temphumid_id` DESC LIMIT 1";
			$result3 = mysqli_query($conn, $sql3);
		    while($data3 = mysqli_fetch_array($result3, MYSQL_NUM))
		    {
				...
				if($humid>40){
					...
					$humid = 27+(27-$humid)*0.5;
				}

				$humid=round($humid, 2);    

		    	$co2 = $data3[3];
		    	$dust = 6+ mt_rand(203, 805)/100;
	...
	// $_7day_ago  = date("Y-m-d H:i:s" , strtotime($date."-7 days"));
	// $_7day_ago2  = date("Y-m-d H:i:s" , strtotime($_7day_ago."+60 second"));
