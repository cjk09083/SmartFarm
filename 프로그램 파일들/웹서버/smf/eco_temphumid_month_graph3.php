<?
    ...
    if(isset($_GET['year_time']) && isset($_GET['month_time']) && isset($_GET['day_time']) )
    {
        ...
        $sql = "SELECT AVG(temp) AS temp_avg, 
                      ...
                      AVG(co2) AS co2_avg, 
                       AVG(dust) AS dust_avg,
                        AVG(out_temp) AS out_temp_avg, 
                      AVG(out_humid) AS out_humid_avg, 
                       AVG(out_dust) AS out_dust_avg,
                      created_at
                FROM temphumid2 where device_id = '$device_id' and created_at between '$time_start' and '$time_end' GROUP BY date_format(created_at, '%Y%m%d')";
        ...
        while($data = mysqli_fetch_array($result))
        {
            ...
            $out_temp_avg = round($data['out_temp_avg'],2);
            $out_humid_avg = round($data['out_humid_avg'],2);
            $out_dust_avg = round($data['out_dust_avg'],2);
            ...
            $created_at = $data['created_at'];
                            ...
                            array_push($array1, 
                                array('dust'.$index =>$dust_avg,
                                    ...
                                    'co2'.$index =>$co2_avg,
                                    'out_temp'.$index =>$out_temp_avg,
                                    'out_humid'.$index =>$out_humid_avg,
                                    'out_dust'.$index =>$out_dust_avg,
